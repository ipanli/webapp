...
var index = 0, classs = ['layermbox'], PanLi = function(options){
  var that = this;
  that.config = ready.extend(options);
  that.view();
};

PanLi.prototype.view = function(){
  ...
  if(!config.type || config.type === 2){
    ...
    if(dialen >= 1){
      PL.close(dialogs[0].getAttribute('index'))
    }
  }
...
PanLi.prototype.action = function(config, elem){
  var that = this;
  ...
  var btn = function(){
    ...
    if(type == 0){
      ...
      config.yes ? config.yes(that.index) : PL.close(that.index);
    }
  };
  if(config.btn){
  ...
  if(config.shade && config.shadeClose){
    ...
    ready.touch(shade, function(){
      PL.close(that.index, config.end);
    });
  }
...
win.PL = {
  ...
  closeAll: function(){
    ...
    for(var i = 0, len = boxs.length; i < len; i++){
      PL.close((boxs[0].getAttribute('index')|0));
    }
  }
};
